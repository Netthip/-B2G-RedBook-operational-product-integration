...
### `HL-027` — Hardening Pass 6 (ตอบ Bo `5615776981`)

| หัวข้อ | สาระ |
|---|---|
| P5-1 | หน้าจัดทำเฉลยและหน้าล็อกใช้ `BlindVerification` — ชั้นเครื่อง = ผนึกแล้ว/ยังไม่ผนึก · `/audit` ปิดบังฟิลด์ผลเครื่องของรอบที่ยังไม่ตรวจผ่าน (ตอนแสดงเท่านั้น · chain ไม่ถูกแตะ) · เทสต์ sentinel 2917/1373 |
| P5-2 | `detector-snapshot-0.3.0` · projection จาก schema จริงทุกคอลัมน์ของสี่ส่วน · ข้อยกเว้นเดียว `local_path_private` · mutation 15 กรณี 5 กลุ่มผ่าน route จริง + กรณีควบคุม · คอลัมน์อนาคตถูกครอบอัตโนมัติ |
| เทสต์ | `1127 passed · 0 failed · 0 skipped` (unit/synthetic 916 · private 211) · **developer-machine reported result · ไม่มี CI** · ทำลายโค้ดชั่วคราว ⇒ 17 เทสต์ล้มตรงกลุ่ม |

**ขอคำยืนยันจาก Bo หนึ่งข้อ** — หน้าล็อกยังแสดงหนึ่งบิต "ผนึกครบแล้วแต่ตรวจไม่ผ่าน ต้องเปิดรอบใหม่" (ไม่บอกว่าอะไรเปลี่ยน)
เพื่อให้กติกา "ห้ามซ่อมทับ" ใช้งานได้ · ถ้าต้องการให้ไม่มีบิตนี้เลย ต้องกำหนดทางอื่นให้ผู้ใช้รู้ว่าต้องเปิดรอบใหม่

🔴 **ห้ามอ่านว่า "freeze blockers closed"** — การรับรองเป็นของ Bo · สถานะทางการยังเป็น **`FREEZE BLOCKED`** ·
ยังไม่ tag · ไม่ merge `main` · ไม่ใช้เอกสารจริง · ไม่มีเฉลยของงานวิจัย · ไม่ scoring/unblind · ไม่อ้างผลการวัดประสิทธิภาพ ·
