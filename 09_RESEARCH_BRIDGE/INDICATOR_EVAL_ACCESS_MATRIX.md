...
## 9. Pass 5 — ทุกทางที่เห็นผลของเครื่องก่อนผนึก (แก้จาก §8)

> append-only · §1–§8 ไม่ถูกแก้

§8 ระบุเฉพาะหน้าจัดทำเฉลย · Bo (`5606392284` P4-1) ชี้ว่า **หน้ารายงานของรอบเปิดผลเครื่องได้ตลอด**
และหน้าจัดทำเฉลยมีลิงก์พาไปตรง ๆ · ตรวจแล้วพบทางอื่นในกลุ่มเดียวกันอีกสามทาง ปิดทั้งหมดในรอบนี้

| ทาง | ก่อนผนึกครบ (ตั้งแต่ Pass 5) |
|---|---|
| หน้ารายงานของรอบ | 🔒 409 |
| หน้ารายการรอบ — จำนวนข้อสังเกต/คู่ของเครื่อง | 🔒 ซ่อน |
| หน้าหลักฐานที่ชี้ข้อสังเกตของเครื่อง (`obs=`) | 🔒 409 |
| หน้าเทียบข้างกัน · ขั้นตัดสิน · ส่งคำตัดสิน · สร้างชุด | 🔒 409 (ตรวจก่อนประกอบคู่) |
| การเทียบความครอบคลุม (`detector_coverage`) | 🔒 ปฏิเสธ |
| ดาวน์โหลดไฟล์ส่งออก | 🔒 409 (ตรวจทุกครั้ง) · route เดิม 410 |
| หน้าเอกสาร/หน้ากระดาษที่ไม่ชี้ข้อสังเกต | ✅ เปิดได้ (ขั้นเฉลยต้องใช้) |
| ทะเบียนที่คนกรอก · ขั้นจัดทำเฉลย | ✅ เปิดได้ — **ไม่มีลิงก์ไปผลของเครื่อง** |

ด่านที่บังคับ: เทสต์ route ทั้งสี่สภาวะ (0-of-N · ตรึงจักรวาลอย่างเดียว · ผนึกเครื่องอย่างเดียว · ผนึกไม่ตรง)
+ ด่าน AST ว่า handler ใดที่ประกอบผลเครื่องต้องตรวจผนึก **ก่อน** บรรทัดที่ประกอบ

🔴 ยังเป็นด่านเชิงลำดับและความสมบูรณ์ของข้อมูล ไม่ใช่ access control — ยังไม่มีบัญชีผู้ใช้


---

## 10. Pass 6 — ขั้นเฉลยรู้เรื่องผลเครื่องได้ **หนึ่งอย่าง: ผนึกแล้วหรือยัง** (แก้จาก §9)

> append-only · §1–§9 ไม่ถูกแก้

§9 ปิดลิงก์และ route แล้ว แต่ Bo (`5615776981` P5-1) ชี้ว่าหน้าจัดทำเฉลย **ยังแสดงจำนวนคู่ของเครื่อง** ·
ตรวจแล้วพบอีกสองทางในกลุ่มเดียวกัน — หน้า `/audit` (มี `by_status` การกระจายสถานะที่เครื่องเสนอด้วย) และหน้าล็อก

| สิ่งที่เกี่ยวกับผลเครื่อง | ก่อนตรวจผ่านครบ (ตั้งแต่ Pass 6) |
|---|---|
| ผนึกแล้ว / ยังไม่ผนึก | ✅ |
| "ผนึกครบแล้วแต่ตรวจไม่ผ่าน — ต้องเปิดรอบใหม่" (หนึ่งบิต ไม่บอกว่าอะไรเปลี่ยน) | ✅ |
| จำนวนคู่ · จำนวนข้อสังเกต · การกระจายสถานะ | ❌ ทุกหน้า รวม `/audit` |
| แฮชผนึกของเครื่องทุกส่วน | ❌ |
| เหตุผลเฉพาะที่ผนึกเครื่องตรวจไม่ผ่าน | ❌ (อยู่ใน log ของเครื่องเท่านั้น) |

ด่านที่บังคับ: เทสต์แบบ **sentinel** — เติมแถวหลอกจนจำนวนคู่/ข้อสังเกตเป็นตัวเลขเฉพาะ แล้วตรวจว่าไม่ปรากฏในทุกหน้า ·
มีกรณีควบคุมว่าหลังตรวจผ่านครบ รายละเอียดใน audit กลับมาแสดง

🔴 ยังเป็นด่านเชิงลำดับและความสมบูรณ์ของข้อมูล ไม่ใช่ access control — ยังไม่มีบัญชีผู้ใช้ · log ในเครื่องยังอ่านได้
