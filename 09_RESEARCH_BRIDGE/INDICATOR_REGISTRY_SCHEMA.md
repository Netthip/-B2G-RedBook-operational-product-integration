...
## 9. Pass 4 — ทะเบียนข้อสังเกตอิสระ และการ **ตรวจ** ผนึก (schema v4)

> append-only ตาม `BO REVIEW COMPLETE — PRE-FREEZE CORRECTIONS PASS 3` (`5605652708`) ·
> หัวข้อ 1–8 ข้างบน **ไม่ถูกแก้**

### 9.1 P3-2 — จักรวาลของเฉลยเลิกตั้งต้นจากสิ่งที่เครื่องสกัดได้

Pass 3 สร้างจักรวาลจาก `ind_observations` ซึ่ง pipeline เดียวกับ detector เป็นผู้บันทึก
⇒ ตัดการพึ่ง **การจับคู่** ได้ แต่ยังพึ่ง **การสกัด** · ตัวชี้วัดที่เครื่องอ่านไม่เจอจะไม่มีวัน
เข้าจักรวาล ทำให้ recall ดูสูงเกินจริงทั้งที่ด่านครบถ้วนขึ้นผ่านครบ

| ตาราง | หน้าที่ |
|---|---|
| `ind_source_inventory` | รายการที่ **คนอ่านเอกสารแล้วกรอกเอง** — ฝั่ง · แฮชเอกสารเต็ม · หน้า · ลำดับที่ปรากฏ · จุดอ้างอิงสั้น · ข้อความ · ผู้กรอก |
| `ind_source_inventory_retractions` | การถอนรายการที่กรอกผิด — **ก่อนตรึงเท่านั้น** แถวเดิมยังอยู่ในประวัติ |
| `ind_runs.old_document_sha256` / `new_document_sha256` | แฮชเนื้อเอกสาร **เต็ม** บันทึกตั้งแต่สร้างรอบ เพื่อให้ทะเบียนอิสระผูกกับเอกสารได้โดยไม่ต้องอ่านผลของเครื่อง |

`build_key_universe()` อ่านจากทะเบียนนี้เท่านั้น · `_universe_records()` เดิมเหลือไว้ให้
`detector_coverage()` ใช้อ่าน "สิ่งที่เครื่องสกัดได้" เพื่อนำไป **เทียบ** เท่านั้น

**กติกาเทียบ** — (ฝั่ง · แฮชเอกสาร · หน้า · ข้อความรูปมาตรฐาน · ลำดับที่ปรากฏ)
ไม่ใช่ `identity_key` เพราะสมอของสองฝ่ายคนละรูป (`line:{index}` กับ `manual:{n}`)
🔴 `NO_MATCH` = "เครื่องไม่ได้ให้ข้อสังเกตที่เทียบเท่ากัน" — จะเป็นการสกัดตกหล่นจริงหรือ
การตัดบรรทัดต่างกัน **เป็นคำตัดสินของคน** · `detector_coverage()` **ไม่ใช่คะแนน**

### 9.2 P3-1 — แฮชที่ผนึกไว้กลายเป็นด่านจริง

เดิม `adjudication_unlocked()` เช็กเพียงว่ามีแถว seal · ตั้งแต่ Pass 4 **คำนวณใหม่จากแถว
ปัจจุบันแล้วเทียบ** ด้วย `hmac.compare_digest` ทั้งสาม artifact ทุกครั้งที่จะเปิดขั้นตัดสิน

| artifact | คำนวณใหม่จาก | รหัสเหตุผลเมื่อไม่ตรง |
|---|---|---|
| `UNIVERSE` | แถวจักรวาล **และ** ทะเบียนอิสระที่มีผล | `UNIVERSE_DIGEST_MISMATCH` · `UNIVERSE_ROWS_DIFFER_FROM_INDEPENDENT_INVENTORY` · `UNIVERSE_ITEM_COUNT_MISMATCH` |
| `KEY` | คำตอบที่มีผล + จักรวาล + แฮชเอกสาร + เวอร์ชัน + manifest | `KEY_DIGEST_MISMATCH` · `KEY_ANSWER_COUNT_MISMATCH` · `KEY_COVERAGE_NO_LONGER_CLOSED` |
| `DETECTOR_OUTPUT` | แถว `ind_mappings` + `rules_hash` | `DETECTOR_OUTPUT_DIGEST_MISMATCH` · `DETECTOR_PAIR_COUNT_MISMATCH` · `DETECTOR_RULES_HASH_MISMATCH` |

🔴 **ห้ามซ่อมทับ** — เมื่อไม่ตรงแล้ว การผนึกซ้ำ การตรึงซ้ำ การกรอกคำตอบ และการกรอก
ทะเบียนอิสระ ถูกปฏิเสธทั้งหมด · ทางเดียวคือ **เปิดรอบประเมินใหม่**

### 9.3 ลำดับที่ระบบบังคับ (ครบทุกขั้น)

```
กรอกทะเบียนอิสระ → ตรึงจักรวาล → ผนึกผลลัพธ์ของเครื่อง
    → ตอบเฉลยให้ครบ → ผนึกเฉลย → ตรวจผนึกสามชั้น → เปิดขั้นตัดสิน/การเทียบผล
```

ป้ายเมื่อยังไม่ครบหรือตรวจไม่ผ่าน: `ADJUDICATION_LOCKED_KEY_OR_DETECTOR_NOT_SEALED`
พร้อมรายการเหตุผลรายชั้นบนหน้าจอ

### 9.4 ข้อจำกัดที่ประกาศไว้เพิ่ม

* **ต้นทุนแรงงาน** — ไม่มีปุ่ม "นำเข้าจากสิ่งที่เครื่องสกัดได้" โดยเจตนา ⇒ เอกสารจริงหลายร้อยหน้า
  ต้องใช้เวลากรอกมาก · เป็นคำตัดสินเชิงกระบวนการที่ต้องยอมรับหรือปรับขอบเขตของ endpoint
* **รอบก่อน v4** ไม่มีแฮชเอกสารระดับรอบ ⇒ กรอกทะเบียนอิสระไม่ได้ ต้องเปิดรอบใหม่
* **หนึ่งต่อหลาย** ยังเป็น `AMBIGUOUS_REVIEW_REQUIRED` สองฝั่ง — ก่อน scoring จริงต้องแยกกองรายงาน
  ต่างหาก หรือเพิ่มความสัมพันธ์ split/merge (ยังไม่ทำ อยู่นอกขอบเขตที่ Bo bound ไว้)


---

## 10. Pass 5 — ขอบเขตการเปิดผลเครื่อง · manifest ไฟล์ส่งออก · ผนึกเครื่องครบ (schema v5)

> append-only ตาม `BO REVIEW COMPLETE — PRE-FREEZE CORRECTIONS PASS 4` (`5606392284`) · หัวข้อ 1–9 ไม่ถูกแก้

### 10.1 P4-3 — ผนึกผลลัพธ์เครื่องครอบข้อสังเกตที่สกัดแล้ว

`detector-snapshot-0.2.0` ผูกสามส่วนแยกกัน แล้วรวมเป็นแฮชเดียว

| ส่วน | ครอบ |
|---|---|
| `mappings_digest` | คู่ทุกคู่ · สถานะที่เสนอ · คะแนน · เหตุผลของเครื่อง · **จำนวนคู่** |
| `observations_digest` | **ข้อสังเกตที่สกัดทุกตัว** — ฝั่ง · แฮชเอกสารเต็ม · หน้า · ลำดับบรรทัด · สมอ · แฮชข้อความดิบและรูปมาตรฐาน · ธงเรขาคณิต · ระดับความมั่นใจของตำแหน่ง · กุญแจตัวตน |
| `metadata_digest` | เวอร์ชัน extractor / normalize / mapping · text backend · กฎและแฮชกฎ · แฮชเอกสารเต็มสองฝั่ง · จำนวนหน้า · page inventory · n_old / n_new / n_pairs |

ผนึกที่ทำด้วยพิธีการเก่า (Pass 3–4) ⇒ `DETECTOR_SNAPSHOT_PROTOCOL_OUTDATED` — ล็อก ไม่ยอมรับเงียบ

### 10.2 P4-4 — นิยามของ "ครั้งที่ปรากฏ" และการเทียบ

`source-inventory-0.2.0` — occurrence = **ข้อความเดียวกันปรากฏเป็นครั้งที่เท่าไรในหน้านั้น นับจากบนลงล่าง**
ฝั่งเครื่องนับด้วย `(line_index, observation_id)` ภายในฟังก์ชันเอง (ไม่พึ่งลำดับจากฐานข้อมูล) ·
เทียบด้วย (ฝั่ง · แฮชเอกสาร · หน้า · ข้อความรูปมาตรฐาน · ครั้งที่ปรากฏ) **ตรงตัวทั้งห้า** ·
เลขหน้าต้องอยู่ใน `1…จำนวนหน้าจริง` ที่บันทึกไว้ในรอบ (จากทะเบียนเอกสาร) · ไม่ทราบจำนวนหน้า ⇒ ปฏิเสธ

### 10.3 P4-2 — `ind_export_manifests`

หนึ่งแถวต่อหนึ่งไฟล์ — ชื่อไฟล์ · package · รอบ · sha256 ของไฟล์ · digest ของผนึกสามชั้น ณ ตอนสร้าง ·
